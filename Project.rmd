...
```{r}
# SENGKANG
sk <- housing %>% dplyr::filter(town == "SENGKANG")
...
```{r}
# PUNGGOL
pg <- housing %>% dplyr::select(town,flat_type,flat_model, floor_area_sqm, resale_price) %>% 
...
```{r}
# WOODLANDS
wl <- housing %>% dplyr::select(town,flat_type,flat_model, floor_area_sqm, resale_price) %>% 
...
```{r}
# Linear Regression for Sengkang
sk <- sk %>% dplyr::filter(flat_type == "4 ROOM" & flat_model == "Model A")
sk_lm <- lm(log(resale_price) ~ floor_area_sqm, data = sk)

summary(sk_lm)

ggplot(data = sk, aes(x = floor_area_sqm, y = log(resale_price))) + 
  geom_point() +
  stat_smooth(method = "lm")
```

```{r}
pg <- pg %>% dplyr::filter(flat_type == "4 ROOM" & flat_model == "Model A")
pg_lm <- lm(resale_price ~ floor_area_sqm, data = pg)

summary(pg_lm)

ggplot(data = pg, aes(x = floor_area_sqm, y = resale_price)) + 
  geom_point() +
  stat_smooth(method = "lm")
```

```{r}
wl <- wl %>% dplyr::filter(flat_type == "4 ROOM" & flat_model == "Model A")
wl_lm <- lm(resale_price ~ floor_area_sqm, data = wl)

summary(wl_lm)

ggplot(data = wl, aes(x = floor_area_sqm, y = resale_price)) + 
  geom_point() +
  stat_smooth(method = "lm")
```
...
```{r}
houses <- housing_dat %>% dplyr::filter(flat_type == "4 ROOM" & flat_model == "Model A")

houses_lm <- lm(resale_price ~ floor_area_sqm, data = houses)

summary(houses_lm)


ggplot(data = houses, aes(x = floor_area_sqm, y = resale_price)) + 
  geom_point() +
  lines(houses_lm$fitted.values)

plot(houses_lm$fitted.values)
...
Origin of data set for Singaporean homes. Kaggle.com.
