...
merged <- left_join(housing, cpi, by = c("month" = "MTH"))  %>% 
            rename(cpi=Value) %>% 
            mutate(resale_adj = (resale_price/cpi) *100)
```
...
p2 <- ggplot(sample_housing) +
        geom_point(aes(x=floor_area_sqm,y=log(resale_price)),
                   size=0.5,position="identity") +
        theme_minimal()
...
housing_dat <- housing %>% 
  dplyr::filter(town %in% randomTown & month == "2019-04-01")
randomTown
...
housing_dat <- housing_dat %>% 
                  filter(flat_model == top_model & flat_type ==top_type)

housing_lm <- lm(log(resale_price) ~ floor_area_sqm, data = housing_dat)

summary(houses_lm)

housing_dat$predicted <- predict(housing_lm)

# Plot the scatter plot and the line representing the linear regression model
ggplot(data = housing_dat, aes(x = floor_area_sqm, y = log(resale_price))) + 
  geom_point() +
