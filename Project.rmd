...
p1 <- ggplot(sample_housing) +
        geom_point(aes(x=floor_area_sqm,y=resale_adj),
                   size=0.5,position="identity") +
        theme_minimal()
p2 <- ggplot(sample_housing) +
        geom_point(aes(x=floor_area_sqm,y=log(resale_adj)),
                   size=0.5,position="identity") +
        theme_minimal()
...
#Assumptions on normality of residuals 
epsilon <- data.frame(resid=housing_lm$residuals,month=housing_dat$month)
ggplot(epsilon) +
  geom_histogram(aes(x=resid))


#Comparison of resale and predicted values
ggplot(housing_dat) + 
