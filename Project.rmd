...
```{r}
X <- housing %>% dplyr::select(block,floor_area_sqm,remaining_lease_mth,
                          is_2_room,is_3_room,is_4_room,is_5_room,is_executive,is_1_room,
                          is_multi_generation,is_north,is_south,is_west,is_east) %>% 
              as.data.frame
X
...
predictions <- predict(ridge_model, newx = X)  # Adjust s (lambda) as needed
